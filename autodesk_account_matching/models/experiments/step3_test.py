...
def extract_features(dbt, session, meta_data, dfs):
    ...
    return session.table("AUTODESK_ACCOUNT_MATCHING_DB.TEST.STEP3_ALL_COLUMN_PAIR_FEATURES")

def model(dbt, session):
    dbt.ref('step2_test')  # Make it so this runs after step2
    dbt.config(
        packages=['snowflake-snowpark-python','pandas','tqdm','httpx','rapidfuzz','langdetect','snowflake-ml-python'],
        python_version="3.11"
    )
    dfs = load_data(session)
    meta_data = session.table("AUTODESK_ACCOUNT_MATCHING_DB.TEST.STEP2_COLUMN_METADATA_DESCRIPTIONS")
