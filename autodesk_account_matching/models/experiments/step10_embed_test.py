...
def apply_llm_judgment_on_account_matches(session, matches, llm_model, llm_temp, llm_top_p, llm_max_tokens):
    ...
    out_df.write.mode("overwrite").save_as_table('AUTODESK_ACCOUNT_MATCHING_DB.TEST.STEP10_FINAL_LLM_ROW_MATCHES_EMBED')
    ...
    return session.table('AUTODESK_ACCOUNT_MATCHING_DB.TEST.STEP10_FINAL_LLM_ROW_MATCHES_EMBED')

def model(dbt, session):
    dbt.ref('step7_8_9_embed_test')  # Make it so this runs after step9
    dbt.config(
        packages=['snowflake-snowpark-python','pandas','tqdm','httpx','rapidfuzz','langdetect','snowflake-ml-python'],
        python_version="3.11"
    )
    matches = session.table('AUTODESK_ACCOUNT_MATCHING_DB.TEST.STEP9_FINAL_TRANSFORMED_DFS_EMBED')
    ...
    if llm_top_n <= 0:
        ...
        out_df.write.mode("overwrite").save_as_table('AUTODESK_ACCOUNT_MATCHING_DB.TEST.STEP10_FINAL_LLM_ROW_MATCHES_EMBED')
        matches = session.table('AUTODESK_ACCOUNT_MATCHING_DB.TEST.STEP10_FINAL_LLM_ROW_MATCHES_EMBED')
        ...
        combined.write.mode("overwrite").save_as_table('AUTODESK_ACCOUNT_MATCHING_DB.TEST.STEP10_FINAL_LLM_ROW_MATCHES_EMBED')
        matches = session.table('AUTODESK_ACCOUNT_MATCHING_DB.TEST.STEP10_FINAL_LLM_ROW_MATCHES_EMBED')
