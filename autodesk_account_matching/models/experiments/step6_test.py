...
def get_best_column_matches(dbt, session, result_df_filtered, dfs):     
    ...
    return session.table("AUTODESK_ACCOUNT_MATCHING_DB.TEST.STEP6_FINAL_COLUMN_MATCHES")

def model(dbt, session):
    dbt.ref('step5_test')  # Make it so this runs after step5
    dbt.config(
        packages=['snowflake-snowpark-python','pandas','tqdm','httpx','rapidfuzz','langdetect','snowflake-ml-python'],
        python_version="3.11"
    )
    dfs = load_data(session)
    result_df_filtered = session.table("AUTODESK_ACCOUNT_MATCHING_DB.TEST.STEP5_GPT_COLUMN_PAIR_CLASSIFICATION")
