...
def apply_gpt_decision(dbt, session, result_df_filtered):
    ...
    return session.table("AUTODESK_ACCOUNT_MATCHING_DB.TEST.STEP5_GPT_COLUMN_PAIR_CLASSIFICATION")

def model(dbt, session):
    dbt.ref('step4_test')  # Make it so this runs after step4
    dbt.config(
        packages=['snowflake-snowpark-python','pandas','tqdm','httpx','rapidfuzz','langdetect','snowflake-ml-python'],
        python_version="3.11"
    )
    result_df_filtered = session.table("AUTODESK_ACCOUNT_MATCHING_DB.TEST.STEP4_FILTERED_COLUMN_PAIRS")
